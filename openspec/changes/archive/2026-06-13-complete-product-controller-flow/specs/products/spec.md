...
The system SHALL expose a public paginated product listing through `GET /products` that returns only products visible to shoppers.
...
- THEN the system returns only products whose status is active and approval status is approved
- AND includes pagination metadata.

#### Scenario: Public list delegates through product service

- GIVEN a public product list request includes supported query parameters
- WHEN `GET /products` is called
...
The system SHALL expose public product details through `GET /products/{id}` only for visible products.
...
#### Scenario: Public detail delegates through product service

- GIVEN a public product detail request includes a product id
- WHEN `GET /products/{id}` is called
- THEN the controller passes the id to the public product detail service method
- AND the response does not expose admin-only management fields.

### Requirement: Admin product CRUD

The system SHALL allow admins to create, read, update, delete or archive, and manage products through `/admin/products` endpoints with validation for identity, category assignment, pricing, status, approval state, images, and inventory fields, while delegating inventory item persistence and inventory movement recording to inventory-owned behavior.
...
- AND delegates inventory initialization to inventory-owned behavior
...
- AND delegates inventory field updates to inventory-owned behavior when inventory fields are provided
...
#### Scenario: Admin controller protects product management routes
...
- AND product creation, update, or deletion is not performed.

#### Scenario: Admin controller delegates product management actions

- GIVEN an authenticated admin calls an `/admin/products` endpoint
- WHEN the request reaches the admin product controller
- THEN the controller passes route parameters, query DTOs, and body DTOs to the matching product service method
- AND returns the service response without changing the product business result.

### Requirement: Admin product listing management

The system SHALL expose an admin product listing through `GET /admin/products` that supports pagination, search, filters, sorting, and management summary fields.
